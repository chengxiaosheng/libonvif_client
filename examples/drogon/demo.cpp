    ...
    media_client->GetStreamUri(request,
        [media_client](OnvifResult<trt_GetStreamUriResponse>&& result) {
            if (result.is_error()) {
                ...
                if (resp.MediaUri.Timeout.duration().count() > 0) {
            ...
            (OnvifResult<tse_GetEventSearchResultsResponse>&& result) {
                if (result.is_error()) {
                    ...
                    for (const auto& event_result : result_list.Result) {
                        ...
                        const auto& data_elem = event_result.Event.Message._any_;
                        if (data_elem.local_name == "Data") {
                            auto name_val = find_simple_item_value(data_elem, "IsDataPresent");
                            if (!name_val.empty()) {
                                bool is_present = (name_val == "true");
                                auto track = find_simple_item_value(data_elem, "Track");
